...
| Cartella | Contenuto |
|---|---|
| `00-Inbox/` | Note grezze da smistare. Tutto entra da qui — anche i file esterni da assimilare (PDF, docx, testo), come copie usa-e-getta: `/distilla` li trasforma in note di conoscenza e poi li elimina. Un file non-markdown in inbox non è mai una nota dell'utente. |
...
2. **Dashboard**: le sezioni "📁 Progetti attivi" e "📥 Inbox" sono viste live
   ...
   🧬 proposte, riga "Ultimo aggiornamento" — che dal 2026-09-04 vive dentro il
   callout ripiegato "Storico degli aggiornamenti", non più in testa al file).
   **Zona fissa in cima, non toccarla**: fra il titolo `# 🧠 Dashboard` e la
   sezione `## 🎯 Oggi` stanno il link alla **dashboard interattiva** (quella coi
   pulsanti), i link di sistema e lo storico ripiegato. Quel blocco **non si
   sposta, non si allunga, non accoglie testo nuovo**. Il link sta in cima
   apposta: prima veniva dopo la riga "Ultimo aggiornamento", che si allunga a
   ogni run, e finiva sepolto sempre più in basso finché l'utente non lo trovava
   più. Qualsiasi cosa tu debba aggiungere va in 🎯 Oggi, 📰 Ultimo brief o 🧬.
